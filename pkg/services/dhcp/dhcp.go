...
func handler(configuration *types.Configuration) server4.Handler {
	return func(conn net.PacketConn, peer net.Addr, m *dhcpv4.DHCPv4) {
		...
		reply.YourIPAddr = net.ParseIP("192.168.127.10")
...
func Serve(c net.PacketConn, configuration *types.Configuration) error {
	server, err := server4.NewServer("", nil, handler(configuration), server4.WithConn(c))
	if err != nil {
		return err
	}
